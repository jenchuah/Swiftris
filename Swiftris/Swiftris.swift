...
class Swiftris {
    ...
    // Adds the falling shape to the collection of blocks maintained by Swiftris. Once the falling shape's blocks are part of the game board, falling shape is nullified and the delegate is notified of a new shape settling into the game board
    func settleShape() {
        if let shape = fallingShape {
            for block in shape.blocks {
    ...
    // A shape should settle when one of the shapes' bottom blocks is located immediately above a block on the game board or when one of those same blocks has reached the bottom of the game board.
    ...
    //#1 Function returns tuple of two arrays - linesRemoved maintains each row of blocks which the user has filled in completely
    func removeCompletedLines() -> (linesRemoved: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>) {
        var removedLines = Array<Array<Block>>()
        for var row = NumRows - 1 ; row > 0; row-- {
            var rowOfBlocks = Array<Block>()
            
            //#2 Iterates columns 0-9 to add every block in a given row to rowOfBlocks. If it ends up with a full set (10 blocks in total), it counts that as a removed line and adds it to the return variable.
            for column in 0..<NumColumns {
                if let block = blockArray[column, row] {
                    rowOfBlocks.append(block)
                }
        ...
        //#3 If no completed lines at all, return empty arrays immediately
        if removedLines.count == 0 {
            return ([],[])
        }
        
        //#4 Level up if points exceed their level * 1000
